...
- `providers/clawhub/skills/goshippo/SKILL.md.template` is the **editable** source. It contains hand-curated framing sections (Setup, Error Handling, Security & Data Transparency) and `{{skill:<name>}}` placeholders for the 10 workflow/mode skills.
...
|---|---|
| A workflow body (steps, tables, examples for any of the 10 skills) | Canonical `skills/<name>/SKILL.md`. The change automatically lands in the digest on next `npm test`. |
...
| The `tracking-map` report template or its references (carrier-tokens, geocoding, data-quirks) | Canonical `skills/tracking-map/assets/` and `skills/tracking-map/references/`. `scripts/build-clawhub-bundle.js` also ships these into the bundle so the digest's Tracking Map section can render. |
