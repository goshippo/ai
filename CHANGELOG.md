...
### Added
- New `tracking-map` skill: turns a package tracking number into a self-contained, interactive HTML report (a MapLibre GL + OpenFreeMap route map plus a synced chronological event timeline; hover a leg for transit time, click a row for the raw event payload) built from the read-only `GetTrack` operation, with a plain-text timeline fallback when the map cannot load. Read-only. The ownership note distinguishes labels not purchased in Shippo (tracking-only, with a possible tracking fee) from labels under a different account (GetTrack-level detail only). Brings the canonical skill count to 10 (1 Decide / 8 Do / 1 Maintain) and adds a "Tracking Map" section to the ClawHub digest, which now also bundles the report template and the skill's references so the digest can render (bundle version 1.4.0 -> 1.5.0). `package.json:version` 1.5.0 -> 1.6.0.
